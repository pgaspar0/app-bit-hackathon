...
public interface ObservationRepository extends JpaRepository<Observation, Integer> {
        ...
        @Query("""
                        ...
                        WHERE (:regiao IS NULL
                               OR UPPER(r.clusterCode) = UPPER(CAST(:regiao AS string))
                               OR UPPER(r.municipio) = UPPER(CAST(:regiao AS string)))
                          AND (:indicador IS NULL
                               OR UPPER(i.indicatorName) = UPPER(CAST(:indicador AS string))
                               OR UPPER(i.category) = UPPER(CAST(:indicador AS string)))
